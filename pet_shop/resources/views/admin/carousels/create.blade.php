...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center space-x-4">
        ...
        <h1 class="text-2xl font-bold text-stone-800">新增輪播圖</h1>
    </div>
    ...
    <div class="bg-white p-8 rounded-2xl shadow-sm border border-stone-100">
        <form action="{{ route('admin.carousels.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            <div>
                <label class="block text-sm font-medium text-stone-700 mb-2">上傳圖片 (建議尺寸: 1920x600)</label>
                <p class="text-xs text-stone-400 mb-2">上傳寬比例圖片效果最佳，系統會自動調整至適合大小並完整顯示。</p>
            ...
            <div>
                ...
                <input type="text" name="title" class="w-full px-4 py-3 rounded-lg border border-stone-100 focus:outline-none focus:ring-2 focus:ring-orange-400 transition" placeholder="輸入標題 (選填)">
            </div>
            <div>
                <label class="block text-sm font-medium text-stone-700 mb-2">描述</label>
                <input type="text" name="description" class="w-full px-4 py-3 rounded-lg border border-stone-100 focus:outline-none focus:ring-2 focus:ring-orange-400 transition" placeholder="輸入描述 (選填)">
            </div>
            <div>
                <label class="block text-sm font-medium text-stone-700 mb-2">排序</label>
                <input type="number" name="order" value="0" class="w-full px-4 py-3 rounded-lg border border-stone-100 focus:outline-none focus:ring-2 focus:ring-orange-400 transition">
            </div>
            <div class="pt-4">
